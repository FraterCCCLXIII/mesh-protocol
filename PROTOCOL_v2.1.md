...
|------|---------|-------------|
| `relationship` | `follow` | Subscribe to entity's content |
| `relationship` | `membership` | Member of group (with role) |
| `relationship` | `subscription` | Paid subscription to entity |
| `relationship` | `block` | Block an entity |
| `interaction` | `react` | Reaction to content (like, emoji) |
| `interaction` | `reply` | Reply indicator (points to content) |
| `interaction` | `progress` | Progress on content (for LMS) |
| `interaction` | `bookmark` | Save for later |
| `credential` | `key_rotation` | Rotate signing keys |
...
// Membership
{"type": "link", "kind": "relationship", "source": "ent:alice", "target": "ent:secret-club",
 "data": {"subkind": "membership", "role": "admin"}}
...
| Feature | FABRIC v2.1 | AT Protocol | Nostr | ActivityPub |
|---------|-------------|-------------|-------|-------------|
